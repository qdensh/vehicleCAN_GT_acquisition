...
#define WHEEL_RADIUS 0.21		       //c11轮胎半径 m
#define WHEEL_BASE   1.435             //轴距 m
#define Tread_BASE   0.93	           //轮距	m
#define Motor_Ratio  30                //驱动电机转速与轮胎转速传动比
#define Steer_Ratio_f (475/22)         //前轮转向电机与前轮转向角传动比  1/Steer_Ratio_f    
...
c11_vcu_status_1_t vcu_status_1_t;
c11_mcuf_torque_feedback_t  mcuf_torque_feedback_t;
c11_mcur_torque_feedback_t  mcur_torque_feedback_t;
c11_sasf_status_t sasf_status_t;
c11_tbox_vehicle_request_status_t tbox_vehicle_request_status_t;
...
int c11_unpack(unsigned int can_id, const uint8_t *src_p, size_t size){
    cout.precision(18);
	
    switch(can_id){
		case C11_VCU_STATUS_1_FRAME_ID:
			c11_vcu_status_1_unpack(&vcu_status_1_t,src_p,size);
			vehicle_speed_vcu = c11_vcu_status_1_vcu_vehicle_speed_decode(vcu_status_1_t.vcu_vehicle_speed);
			break;
        case C11_MCUF_TORQUE_FEEDBACK_FRAME_ID:
            c11_mcuf_torque_feedback_unpack(&mcuf_torque_feedback_t,src_p,size);
			speed_measured_mcuf = c11_mcuf_torque_feedback_speed_measured_decode(mcuf_torque_feedback_t.speed_measured);
			break;
		case C11_SASF_STATUS_FRAME_ID:
			c11_sasf_status_unpack(&sasf_status_t,src_p,size);
			sasf_steering_wheel_angle = c11_sasf_status_sasf_steering_wheel_angle_decode(sasf_status_t.sasf_steering_wheel_angle);
			sasf_steering_wheel_angle_speed = c11_sasf_status_sasf_steering_wheel_angle_speed_decode(sasf_status_t.sasf_steering_wheel_angle_speed);
			break;
        case C11_TBOX_VEHICLE_REQUEST_STATUS_FRAME_ID:
            c11_tbox_vehicle_request_status_unpack(&tbox_vehicle_request_status_t,src_p,size);
            tbox_shift_level_request = c11_tbox_vehicle_request_status_tbox_shift_level_request_decode(tbox_vehicle_request_status_t.tbox_shift_level_request);
            tbox_target_speed = c11_tbox_vehicle_request_status_tbox_target_speed_decode(tbox_vehicle_request_status_t.tbox_target_speed);
            tbox_st_wheel_angle_req = c11_tbox_vehicle_request_status_tbox_st_wheel_angle_req_decode(tbox_vehicle_request_status_t.tbox_st_wheel_angle_req);
            break;	
    }

	vehicle_speed_f = -(speed_measured_mcuf * 2 * M_PI * WHEEL_RADIUS / 60 / Motor_Ratio * 3.6);
    vehicle_speed = vehicle_speed_f;
...
int main(int argc, char **argv)
{
	...
	if(vehicle_record_or_read == "record"){
		...
		myfile << "speed(m/s),steering_angle(deg),steering_angle_velocity(deg/s),";
		myfile <<std::endl;
	}
    ...
    {  	
		...
		for (index = 0; index < receivedNum; index++)
		{
			...
			if(rx_buf_value[index].can_id == C11_VCU_STATUS_1_FRAME_ID || rx_buf_value[index].can_id == C11_MCUF_TORQUE_FEEDBACK_FRAME_ID
            || rx_buf_value[index].can_id == C11_MCUR_TORQUE_FEEDBACK_FRAME_ID || rx_buf_value[index].can_id == C11_SASF_STATUS_FRAME_ID
            || rx_buf_value[index].can_id == C11_TBOX_VEHICLE_REQUEST_STATUS_FRAME_ID){
				if (!rx_buf_value[index].rtr){
					...
					c11_unpack(rx_buf_value[index].can_id,rx_buf_value[index].data,rx_buf_value[index].len);

                    // 过滤噪声 noise
                    // std::cout << "original speed: " << speed << ", original steering_angle: "   << steering_angle * 180 / M_PI << std::endl;;
                    if(abs(vehicle_speed) >= 30 || abs(vehicle_steerAngle_f) >= 30){
                        continue;
                    }
                    // abs(steering_angle) <= 0.3 deg　
                    ...
                    AckermannDriveStamped.drive.steering_angle = float(vehicle_steerAngle_f * M_PI / 180) ;   // rad
                    ...
                    vehicleStamped.vehicle_speed_f = vehicle_speed_f;
                    ...
                    vehicleStamped.speed_measured_mcuf = speed_measured_mcuf;
                    ...
                    if(rx_buf_value[index].can_id != C11_SASF_STATUS_FRAME_ID && 
                    rx_buf_value[index].can_id != C11_MCUF_TORQUE_FEEDBACK_FRAME_ID
                    ){
                        continue;
                    }
                    ...
                    if(vehicle_record_or_read == "record"){
                        if(sequenceNum >= 10){
                            ...
                            myfile << AckermannDriveStamped.drive.speed << delim << AckermannDriveStamped.drive.steering_angle * 180 / M_PI << delim << AckermannDriveStamped.drive.steering_angle_velocity * 180 / M_PI << delim;
                            myfile <<std::endl;	
                        }
					}
